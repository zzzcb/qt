...
public:
    ...
    QList<QtProperty *> m_subItems;		//[zcb0008] 子属性

    QString m_valueToolTip;
    QString m_descriptionToolTip;
    QString m_statusTip;
    QString m_whatsThis;
    QString m_name;						// [zcb0003] 属性的名字
    bool m_enabled;						// [zcb0001] 属性是否被禁用
    bool m_modified;					// [zcb0002] 属性是否被修改过,被修改过的会加粗字体
...
public:
    ...
    QSet<QtProperty *> m_properties; // [zcb0011]该manager 管理所有的QtProperty,一个manager可以管理多个QtProperty 
};
...
*/
bool QtProperty::hasValue() const    // [zcb0005] 属性是否有值,内部找到所属的manager，然后从中查找自己是否有值 | 默认直接返回true, 只有 QtGroupPropertyManager 直接返回false
{
...
*/
QIcon QtProperty::valueIcon() const  //[zcb0006] 属性的图标,内部找到所属的manager,然后从中查找自己的图标 | 默认返回空QIcon(),有QtBoolPropertyManager QtEnumPropertyManager QtFontPropertyManager QtColorPropertyManager QtCursorPropertyManager有相应图标返回
{
...
*/
QString QtProperty::valueText() const //[zcb0007] 属性的文本,内部找到所属的manager,然后从中查找自己的值 | abstract manager 的子类基本都有重写该函数
{
...
void QtProperty::setPropertyName(const QString &text)
{
    ...
    d_ptr->m_name = text;
    propertyChanged(); // [zcb0004] 告知manager 属性改变了, manager 会以信号的方式将当前的property 发射出去  
}
            ...
            QtProperty *afterProperty)
{
    ...
    d_ptr->m_manager->d_ptr->propertyInserted(property, this, properAfterProperty); // [zcb0009] 加入新的子属性后,通过manager将信号发射出去, 参数分别为: 要插入的property|父property|父property已有孩子property
}
...
void QtProperty::removeSubProperty(QtProperty *property)
{
    ...
    d_ptr->m_manager->d_ptr->propertyRemoved(property, this);// [zcb0010] 移除子property，通过manager将信号发射出去  
...
public:
    ...
    QtBrowserItem *m_currentItem;   // [zcb0012] 
};
