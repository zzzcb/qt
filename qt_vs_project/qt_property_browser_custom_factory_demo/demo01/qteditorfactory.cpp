...
{
	QString userInput = displayText();

	if (userInput != m_strVal)
	{
		setText(userInput);
		setValue(userInput);
		emit valueChanged(userInput);
	}
}

// lxQtFilePathFactoryPrivate
class lxQtFilePathFactoryPrivate : public EditorFactoryPrivate<lxQtFilePathEdit>
{
	lxQtFilePathFactory *q_ptr;
	Q_DECLARE_PUBLIC(lxQtFilePathFactory)
public:
	void slotPropertyChanged(QtProperty *property, QString value);
	void slotSetValue(const QString &value);
};

void lxQtFilePathFactoryPrivate::slotPropertyChanged(QtProperty *property, QString value)
{
	const auto it = m_createdEditors.constFind(property);
	if (it == m_createdEditors.cend())
		return;
...
void lxQtFilePathFactoryPrivate::slotSetValue(const QString &value)
{
	QObject *object = q_ptr->sender();
	const EditorToPropertyMap::ConstIterator ecend = m_editorToProperty.constEnd();
	for (EditorToPropertyMap::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
...
lxQtFilePathFactory::lxQtFilePathFactory(QObject *parent)
	: QtAbstractEditorFactory<QtStringPropertyManager>(parent), d_ptr(new lxQtFilePathFactoryPrivate())
{
	d_ptr->q_ptr = this;
}
...
QWidget *lxQtFilePathFactory::createEditor(QtStringPropertyManager *manager,
	QtProperty *property, QWidget *parent)
{
	lxQtFilePathEdit *editor = d_ptr->createEditor(property, parent);
	editor->setValue(manager->value(property));
